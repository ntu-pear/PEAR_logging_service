...
def get_logs_by_param_activity(query: LogQuery, pageNo: int = 0, pageSize: int = 10):
    ...
    if query.activity_id:
        must_conditions.append({
            "bool": {
                "should": [
                    {"match_phrase": {"message": f"'entity_id': {query.activity_id}"}},
                    {"match_phrase": {"message": f"'id': {query.activity_id}"}},
                ],
                "minimum_should_match": 1
            }
        })
    ...
    try:
        ...
        for hit in hits:
            try:
                ...
                if inner_message:
                    if inner_message.get("entity_id"):
                        activity_id = inner_message.get("entity_id")
                    elif original_data.get("id"):
                        activity_id = original_data.get("id")
                    elif updated_data.get("id"):
                        activity_id = updated_data.get("id")
